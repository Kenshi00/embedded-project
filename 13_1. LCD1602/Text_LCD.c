 ...
 * Text_LCD.c
 *
 * Created: 2026-07-01 오전 11:16:19
 *  Author: kccistc
 */ 
#include "Text_LCD.h"
...
void LCD_init(void)
{
	...
	LCD_write_command(COMMAND_8_BIT_MODE);  // 8비트 모드
		...
		LCD_write_command(0x02);                // 커서를 맨앞으로 이동
...
void init_lcd(void)
{
	...
	_delay_ms(15);
	lcd_write_init_nibble(0x03);
	_delay_ms(5);
	lcd_write_init_nibble(0x03);
	_delay_us(150);
	lcd_write_init_nibble(0x03);

	lcd_write_init_nibble(0x02);
...
void lcd_write_init_nibble(uint8_t data)
{
	PORT_CONTROL &= ~(1 << RS_PIN);
	PORT_CONTROL &= ~(1 << RW_PIN);

	PORT_DATA = (PORT_DATA & 0x0F) | ((data & 0x0F) << 4);
